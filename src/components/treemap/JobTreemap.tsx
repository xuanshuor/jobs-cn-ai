...
function buildTileLayouts(nodes: TreemapNode[]): TileLayout[] {
...
export function JobTreemap({
  ...
  const layoutResult = useMemo((): TreemapLayoutResult => {
    ...
    return { tiles: buildTileLayouts(nodes) };
  }, [jobs, width, height, weightMap]);
    ...
    >
      <svg width={width} height={height} role="img" aria-label="职业 Treemap">
        ...
        {tileList.map((t) => {
          ...
          const hovered = hoveredId === job.id;
          ...
          return (
            <g
              ...
              onMouseEnter={(e) => openTip(e.clientX, e.clientY, job)}
              onMouseMove={(e) =>
                setTip((tipPrev) =>
...
function Tooltip({
  ...
  useEffect(() => {
    ...
    const apply = () => setSheetMode(mq.matches);
    apply();
